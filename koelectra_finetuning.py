...
train_data_path = "랜덤_샘플링_utf.csv"

dataset = pd.read_csv(train_data_path).dropna()
text = list(dataset['rivew '].values)
label = dataset['label_again'].values
...
model = ElectraForSequenceClassification.from_pretrained('koelectra-base-v3-discriminator', num_labels=2)
model.to(device)
...
for e in range(0, epoch):
    ...
    model.eval()
    train_loss, train_accuracy, train_steps = 0, 0, 0
...
print("\n\n *** 모델 저장 ***")
save_path = "koelectra_base_criminal_city"
model.cpu()
